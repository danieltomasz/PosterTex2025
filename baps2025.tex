...
\title{\firasemibold The spectral exponent is associated with the synchrony measured via ITPC in gamma ASSRs}
...
\begin{alertblock}{%
	...
	\begin{minipage}[b]{0.9\colwidth}
		Hypothesis.\;
		{\mdseries 	We hypothesize that a steeper slope of the 1/f aperiodic component (i.e., a larger spectral exponent) in MEG signals is associated with higher auditory steady-state response (ASSR) synchrony, as measured by inter-trial phase coherence (ITPC) during 40 Hz auditory stimulation.
		}
	\end{minipage}%
}
\end{alertblock}
...
\begin{textblock*}{0.9\colwidth}(\leftmargin+\colwidth+\colsep, \blockOne)
	...
	% 	\end{center}
		...
		\begin{itemize}
			...
			\item  Simplified formula  describing 1/f component  of the  PSD  of the signal: $AP(f) = b - log(k + f^{\chi})$;  The exponent ${\chi}$ is related to the negative slope of the power spectrum in log-log space, that is, the inverse relationship between power and frequency;  The offset $b$ describe the uniform shift in power across frequencies
...
\TPshowboxestrue
\begin{textblock*}{1.1\colwidth}(\leftmargin+1.9\colwidth+2\colsep,  \blockOne)
	...
	\begin{alertblock}{%
		...
		\begin{minipage}[b]{0.9\linewidth+\colsep-125pt}
			Spectral Parametrization.\;
			{\mdseries Mean values \mbox{of} \mbox{estimated} \mbox{parameters} describing the 1/f part \mbox{of}  \mbox{spectral} \mbox{power} across different regions (right hemisphere). }
		\end{minipage}%
	}
...
\begin{textblock*}{\colwidth}(\leftmargin+2\colwidth+2\colsep, \blockTwo-200pt)
...
\item  For investigation of the the relationship we fit the model $ITPC:stim~Exponent: stim+(1 +Exponent: stim|ROI+(1+Exponent: stim|Subject)$ with using z-scored Exponent. The model was fitted using the \texttt{brms} package and \citep{chen2019a}.
		\end{itemize}
...
\begin{textblock*}{\colwidth}(\leftmargin+2\colwidth+2\colsep, \blockThree-160pt)
